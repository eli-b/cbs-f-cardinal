...
int SingleAgentICBS::extractLastGoalTimestep(const std::vector< std::list< std::pair<int, int> > >& cons)
{
	if (!cons.empty()) 
	{
		for (int t = static_cast<int>(cons.size()) - 1; t > 0; t--) 
		{
			for (auto it = cons[t].begin(); it != cons[t].end(); ++it)
			{
...
inline bool SingleAgentICBS::isConstrained(int curr_id, int next_id, int next_timestep, const ConstraintTable& ct)  const
{
	if (cons.empty())
		return false;
...
bool SingleAgentICBS::findPath(Path &path,
        const ConstraintTable& constraint_table, const CAT& cat, int minLength)
{
	...
	auto start = new LLNode(start_location, 0, my_heuristic[start_location], nullptr, 0, 0, false);
	num_generated++;
	...
	lower_bound = max(constraint_table.length_min, lower_bound);
	...
	while (!focal_list.empty()) 
	{
        ...
        {
            if (curr->parent == nullptr || curr->parent->loc != goal_location)
            {
        ...
        for (int i = 0; i < 5; i++)
		{
            ...
            if (validMove(curr->loc, next_id) && !my_map[next_id] &&
                    ...
                    !constraint_table.is_constrained(curr->loc, next_id, next_timestep, map_size))
			{
                ...
                if (it == allNodes_table.end() ||
                    (next_id == goal_location && constraint_table.length_min > 0))
				{
...
inline void SingleAgentICBS::releaseClosedListNodes()
{
...
SingleAgentICBS::SingleAgentICBS(int start_location, int goal_location,
	...
	start_location(start_location), goal_location(goal_location), my_map(my_map),
    map_size(map_size), num_col(num_col), moves_offset(moves_offset), num_of_conf(0)
{}
