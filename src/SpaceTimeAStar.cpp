							  ...
							  const vector<Path*>& paths, int agent, int lowerbound)
{
	...
	while (!open_list.empty())
	{
		...
		for (int next_location : next_locations)
		{
			...
			bool keep = constraint_table.updateUnsatisfiedPositiveConstraintSet(curr->unsatisfied_positive_constraint_sets,
																				next->unsatisfied_positive_constraint_sets, next_location,
																				next_timestep);
			if (!keep)
			{
