...
public:
	...
	int extractLastGoalTimestep(const std::vector< std::list<std::pair<int, int> > >& cons);
	...
	// inline bool isConstrained(int curr_id, int next_id, int next_timestep, const ConstraintTable& ct) const;
	...
	bool findPath(std::vector<PathEntry> &path, const ConstraintTable& constraint_table,
